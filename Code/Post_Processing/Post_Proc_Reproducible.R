...
for(jj in 1:nSims){
  ...
  for(t in 1:t.T){
    ...
    if(jj==1){
      zeta_13 = read.csv(paste("~/DGLM/Reproducibility/Init_",141,"/Zeta_",t,".csv",sep=""), header=F, stringsAsFactors=F)
      zeta_17 = read.csv(paste("~/DGLM/Reproducibility/Init_",151,"/Zeta_",t,".csv",sep=""), header=F, stringsAsFactors=F)
    }
...
save(file = paste("~/DGLM/Reproducibility/Rep_",INIT_base,".RData",sep=""), Gamma.init, Zeta.init, Theta.init, Eta_Players)
load(file = paste("~/DGLM/Reproducibility/Rep_",INIT_base,".RData",sep=""))
...
for(i in 1:nPlayers){
  tmp = data.frame(Gamma = POI_Gamma[i,], Age = Age, pt025 = POI_Gamma_pt025[i,], pt975 = POI_Gamma_pt975[i,])
  pdf(paste("~/Dropbox/GT_2015/Figures/Gamma_",players[i],".pdf",sep="") )
  g = ggplot(data = tmp, aes(x=Age,y=Gamma,size=1.5 ))+
    geom_point(na.rm=T)+
    geom_line(aes(Age,pt025), linetype=2, size = .5)+
    geom_line(aes(Age,pt975), linetype=2, size = .5)+
    xlab("Age")+
    ylab("Gamma")+
    ylim(1,K)+
  ...
  tmp = data.frame(Zeta = c(POI_Zeta[i,], tmp13, tmp17), Age = rep(Age,times=3), K = factor(KK) ) 
  pdf(paste("~/Dropbox/GT_2015/Figures/Zeta_",players[i],".pdf",sep="") )
  g=ggplot(data = tmp, aes(x=Age,y=Zeta, colour = K ))+
