...
for(i in 1:Age.length){
  age = Age.Range[1]-1 + i
  players = Data$playerID[Data$Age==age & Data$AB>AB_thresh ]
  if(length(players) == 0){
  ...
  for (j in 1:length(players)){
    Player.Prototype = list(Year = NA, Response = list(AB = mat, HR =mat), Predictors = list(F_Park = mat, F_Pos = mat, Salary = NA) )
...
save(file = "~/DGLM/Data/AgeAlignmentSmall.RData", Age.Alignment)
